...
import { CanLoadValueFunc } from '../interfaces.ts';
...
export class MockValueLoader<TValue> implements ValueLoader<TValue> {
  ...
  computeKeyFn: ((entry: DirectoryEntryInContext) => string | undefined) | undefined;
  ...
  #defaultCanLoadValue(entry: DirectoryEntryInContext): boolean | Promise<boolean> {
  ...
  canLoadValue(entry: DirectoryEntryInContext): boolean | Promise<boolean> {
    const result = this.canLoadValueFn ? this.canLoadValueFn(entry) : this.#defaultCanLoadValue(entry);
